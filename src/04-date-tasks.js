...
function timeSpanToString(startDate, endDate) {
  function part(value, length) {
    ...
    const len = length - val.length;
    return ('0'.repeat(len) + val);
  }
...
function angleBetweenClockHands(date) {
  const getHours = date.getUTCHours() % 12;
  const getMinutes = date.getUTCMinutes();
  const eq = 0.5 * ((60 * getHours) - (11 * getMinutes));
  let result;
  if (eq > 180) {
    result = Math.abs((360 - eq) * (Math.PI / 180));
  } else {
    result = Math.abs((eq) * (Math.PI / 180));
  }
  return result;
}
